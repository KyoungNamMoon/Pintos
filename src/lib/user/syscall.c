...
int
openf (const char *file, int flags)
{
  return syscall2 (SYS_OPEN, file, flags);
}
...
bool
setresuid (int ruid, int euid, int suid)
{
...
int
setresgid (int rgid, int egid, int sgid)
{
...
getmode (int fd)
{
  return syscall1 (SYS_GETMODE, fd);
}

int
getowner (int fd)
{
  return syscall1 (SYS_GETOWNER, fd);
}

int
chown (const char *path, int uid, int gid)
{
  return syscall3 (SYS_CHOWN, path, uid, gid);
}
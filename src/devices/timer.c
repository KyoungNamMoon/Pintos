...
static struct spinlock ticks_lock;
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  if (get_cpu ()->id == 0) {
    ...
    spinlock_release (&ticks_lock);
    timer_settime (ticks_snapshot * NSEC_PER_SEC / TIMER_FREQ);
  } else {
    ticks_snapshot = timer_ticks ();
  }
  thread_awake(ticks_snapshot);
  thread_tick ();
}
